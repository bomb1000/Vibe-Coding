# Chinese (Taiwan) - zh_TW

## Title
English Writing Helper

## Summary
把文字翻成自然英文，支援正式、口語與自訂寫作風格。

## Description
English Writing Helper 是給非英語母語者使用的英文寫作助手。你可以在網頁上把文字翻成自然英文，選擇正式寫作、輕鬆口語，或用自訂 prompt 指定你的語氣、格式和目標讀者。適合寫 Email、社群貼文、聊天訊息、筆記、文件和各種日常英文內容。

主要功能

- 即時翻譯：可翻譯選取文字，也可在輸入時輔助轉成英文。
- 語氣選擇：快速切換正式寫作或自然口語。
- 自訂翻譯指令：用自己的 prompt 指定語氣、格式、受眾或特殊寫作規則。
- Gemini/OpenAI 支援：可使用自己的 Google Gemini 或 OpenAI API Key。
- 個人化設定：儲存 API Key、AI 服務商、模型、語氣與自訂 prompt。
- 多語介面：可跟隨 Chrome/系統語言，也可在設定頁手動切換。
- 回饋 / 許願：可直接從插件送出問題回報、使用心得或功能願望。
- 側邊視窗：翻譯結果會出現在方便閱讀的側邊視窗中，不打斷原本工作流程。
- 彈性控制：可隨時開啟或關閉插件。

為什麼安裝 English Writing Helper？

它適合已經知道自己想說什麼，但希望英文更自然、更精準的人。你可以選取文字、翻譯、調整語氣、複製結果，然後繼續寫作。

隱私說明

你的 API Key 會儲存在 Chrome 擴充功能儲存空間中。插件不會自動把目前網址、瀏覽紀錄、API Key、原文或譯文傳送給開發者。選填的匿名用量統計只包含翻譯次數、字數、服務商、語氣、插件版本等彙總資料。
